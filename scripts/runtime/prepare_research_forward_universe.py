 ...
 try:
  ...
  if a.apply:
   ...
   for path in paths:
    ...
    candidate=ForwardCandidate('FORWARD_CANDIDATE_'+fp.path_id.removeprefix('FORWARD_'),path.strategy_reference,fp,path.signal_stock_code,'RESEARCH_EXACT_REPLAY_CANONICAL',datetime.now(),'UNATTENDED_FORWARD_BOOTSTRAP',True)
    try: registry.register(candidate)
    except Exception as error:
     if 'duplicate key' not in str(error): raise
   result['active_forward_candidate_count']=len(paths)
