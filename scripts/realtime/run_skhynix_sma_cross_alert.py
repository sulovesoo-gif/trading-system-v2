...
def main() -> int:
    ...
    try:
        ...
        program_collector = ProgramCollector(KISClient()); execution_collector = StockExecutionCollector(KISClient())
        ...
        while True:
            ...
            execution_schedule = codes.api_schedule("STOCK_EXECUTION_5SEC")
            program_due = schedule.due(now)
            execution_due = execution_schedule.due(now)
            if not active(now) or now.second not in (*SCHEDULED_SNAPSHOT_SECONDS, 1, schedule.execution_second if program_due else -1, now.second if execution_due else -1) or tick == last_dispatch: time.sleep(.2); continue
            last_dispatch = tick
            for stock in codes.enabled_minute_stocks():
                ...
                if program_due and stock.analysis_yn and stock.program_collect_yn:
                    ...
                    continue
                if execution_due and stock.stock_code == "000660" and stock.analysis_yn:
                    try:
                        raw.store(RawTable.STOCK_EXECUTION, execution_collector.collect(stock_code=stock.stock_code, market_code="KOSPI", trading_venue=venue, collect_cycle="5SEC"))
                    except Exception as error:
                        print(f"KIS execution-strength collection failed: {type(error).__name__}")
