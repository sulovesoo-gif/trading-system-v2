...
def ddl_columns(content: str) -> tuple[str, ...]:
    ...
    return tuple(match.group(1) for line in definition.splitlines() if (match := re.match(r"\s{4}([a-z][a-z0-9_]*)\s+(?:TIMESTAMP|DATE|VARCHAR|CHAR|BIGINT|INTEGER|NUMERIC|JSONB)", line)))
...
class RawSpecsDdlTest(unittest.TestCase):
    def test_only_eight_approved_raw_tables_are_registered(self):
        self.assertEqual(set(RAW_SPECS), set(RawTable))
        self.assertEqual(len(RAW_SPECS), 8)
