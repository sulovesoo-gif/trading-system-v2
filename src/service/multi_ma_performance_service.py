...
class MultiMaPerformanceService:
    ...
    def _state(self,key):
        ...
        self._restore_open_trade(key, state)
        self.runtime[key] = state
        return state

    def _restore_open_trade(self, key, state):
        if state.portfolio.direction != "FLAT" or not hasattr(self.repository, "get_open_trade"):
            return
    ...
    def process_signal(self,key,*,signal_no,direction,signal_time,price,reason):
        ...
        # its in-memory state was written.  Re-check before allocating a cycle.
        self._restore_open_trade(key, state)
