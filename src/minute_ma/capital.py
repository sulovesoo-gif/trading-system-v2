...
class PostgresMinuteMaCapitalStore:
    ...
    def apply_settlement(self,*,minute_live_trade_id:int,amounts:SettlementAmounts,
        ...
        with self.connection_factory() as connection,connection.cursor() as cursor:
            ...
            path_id,epoch_no,policy_path_id,policy_operation_id=trade[0],trade[1],trade[4],trade[5]
            cursor.execute("SELECT pg_advisory_xact_lock(hashtext(%s))",
                           (f"minute-ma-capital|{policy_path_id or path_id}|{epoch_no}",))
            cursor.execute("""INSERT INTO minute_ma_live_capital_settlement(
              ...
              (settlement_id,minute_live_trade_id,path_id,epoch_no,amounts.entry_filled_amount,
               ...
               policy_path_id,policy_operation_id))
            created=cursor.fetchone() is not None
            if created:
                capital_table=('minute_ma_policy_compound_capital' if policy_path_id is not None
                               else 'minute_ma_compound_capital')
                identity_column=('minute_policy_path_id' if policy_path_id is not None else 'minute_path_id')
                identity_value=policy_path_id if policy_path_id is not None else path_id
                cursor.execute(f"""UPDATE {capital_table}
                  ...
                  WHERE {identity_column}=%s AND capital_epoch_no=%s""",
                  (amounts.net_realized_pnl,amounts.net_realized_pnl,settled_at,identity_value,epoch_no))
