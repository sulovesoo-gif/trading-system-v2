...
class PostgresMinuteMaNoSendAdapter:
    ...
    def plan_entry(self,*,minute_path_id:int,minute_paper_trade_id:int,
                   ...
                   cash_includes_pending_reservations:bool,source_event_time:datetime,
                   minute_policy_path_id:int|None=None,
                   underlying_entry_reference_price:Decimal|None=None,
                   stop_threshold_price:Decimal|None=None,
                   stop_policy:str|None=None) -> MinuteMaNoSendResult:
        identity=minute_policy_path_id if minute_policy_path_id is not None else minute_path_id
        intent_key=_digest(f"MINUTE_MA_V1|ENTRY|{identity}|{signal_event_key}")
        ...
        with self.connection_factory() as connection,connection.cursor() as cursor:
            ...
            if minute_policy_path_id is not None:
                cursor.execute("""SELECT o.minute_policy_operation_id,o.operation_status,o.capital_epoch_no,
                                          c.strategy_compound_capital
                                     FROM minute_ma_policy_operation o
                                     JOIN minute_ma_policy_compound_capital c
                                       ON c.minute_policy_path_id=o.minute_policy_path_id
                                      AND c.capital_epoch_no=o.capital_epoch_no
                                    WHERE o.minute_policy_path_id=%s AND o.effective_to IS NULL FOR UPDATE""",
                               (minute_policy_path_id,))
            else:
            ...
            cursor.execute("""SELECT skip_reason FROM minute_ma_live_entry_skip
                                WHERE minute_path_id=%s AND signal_event_key=%s
                                  AND minute_policy_path_id IS NOT DISTINCT FROM %s""",
                           (minute_path_id,signal_event_key,minute_policy_path_id))
            ...
            if quantity<=0 or notional>effective_cash:
                ...
                cursor.execute("""INSERT INTO minute_ma_live_entry_skip(
                  ...
                  (str(uuid5(NAMESPACE_URL,"minute-ma-skip|"+intent_key)),minute_path_id,
                   ...
                   minute_policy_path_id,operation_id if minute_policy_path_id is not None else None))
                connection.commit()
                return MinuteMaNoSendResult(reason,intent_key,None,quantity)
            strategy_instance=(f"MINUTE_MA_POLICY_PATH:{minute_policy_path_id}:EPOCH:{epoch_no}"
                               if minute_policy_path_id is not None
                               else f"MINUTE_MA_PATH:{minute_path_id}:EPOCH:{epoch_no}")
            ...
            cursor.execute("""INSERT INTO minute_ma_live_intent(
              ...
              reference_price,requested_quantity,capital_at_signal,lifecycle_status,minute_policy_path_id,
              minute_policy_operation_id,
              underlying_entry_reference_price,stop_threshold_price,stop_policy)
              VALUES (%s,%s,%s,%s,'ENTRY',%s,%s,%s,%s,'NO_SEND_VALIDATED',%s,%s,%s,%s,%s)
              ON CONFLICT(intent_key) DO NOTHING""",
              (intent_id,intent_key,minute_path_id,minute_paper_trade_id,source_event_time,
                price,quantity,capital,minute_policy_path_id,
                operation_id if minute_policy_path_id is not None else None,underlying_entry_reference_price,
               stop_threshold_price,stop_policy))
            cursor.execute("""INSERT INTO live_order_request(
              ...
              VALUES (%s,%s,%s,%s,%s,%s,'BUY',%s,%s,%s,'MARKET_REFERENCE_ONLY',%s,%s,%s,
                      'NO_SEND_VALIDATED','NO_SEND_VALIDATED','MINUTE_MA_SEND_LOCKED',
                       jsonb_build_object('minute_path_id',%s,'minute_policy_path_id',%s,
                                          'operation_id',%s,'minute_policy_operation_id',%s))
              ...
              (request_id,request_key,strategy_instance,intent_id,
               str(uuid5(NAMESPACE_URL,"minute-ma-decision|"+intent_key)),execution_stock_code,
                notional,quantity,price,source_event_time,capital,notional,minute_path_id,
                minute_policy_path_id,None if minute_policy_path_id is not None else operation_id,
                operation_id if minute_policy_path_id is not None else None))
    ...
    def plan_exit(self,*,minute_live_trade_id:int,execution_stock_code:str,
                  ...
                  exit_reason:str,minute_policy_path_id:int|None=None) -> MinuteMaNoSendResult:
        ...
        with self.connection_factory() as connection,connection.cursor() as cursor:
            ...
            cursor.execute("""SELECT t.minute_path_id,t.minute_paper_trade_id,t.ownership_id,t.capital_at_signal,
                                      t.minute_policy_path_id,t.minute_policy_operation_id
            ...
            path_id,paper_trade_id,ownership_id,capital,stored_policy_path_id,policy_operation_id=trade
            if minute_policy_path_id is not None and minute_policy_path_id!=stored_policy_path_id:
                return MinuteMaNoSendResult("POLICY_PATH_MISMATCH",intent_key,None,0)
            minute_policy_path_id=stored_policy_path_id
            ...
            strategy_instance=(f"MINUTE_MA_V1_POLICY:{minute_policy_path_id}:LIVE_TRADE:{minute_live_trade_id}"
                               if minute_policy_path_id is not None
                               else f"MINUTE_MA_PATH:{path_id}:LIVE_TRADE:{minute_live_trade_id}")
            cursor.execute("""INSERT INTO minute_ma_live_intent(
              ...
              VALUES (%s,%s,%s,%s,%s,'EXIT',%s,%s,%s,%s,'NO_SEND_VALIDATED',%s,%s,%s,%s)""",
              (intent_id,intent_key,path_id,paper_trade_id,minute_live_trade_id,source_event_time,price,quantity,capital,
               minute_policy_path_id,policy_operation_id,minute_live_trade_id,exit_reason))
            cursor.execute("""INSERT INTO live_order_request(
              order_request_id,idempotency_key,strategy_instance_id,source_intent_id,source_decision_id,
              execution_stock_code,side,requested_notional,requested_quantity,reference_price,order_type,
              execution_target_time,strategy_capital_before,reserved_capital,safety_status,status,reason,detail)
