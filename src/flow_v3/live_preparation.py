...
def record_preparations(pool):
    with pool.connection() as conn, conn.transaction():
        ...
        result=conn.execute("""INSERT INTO flow_v3_live_preparation_intent
            (operation_id,strategy_id,event_id,paper_trade_id,entry_event_key,status,execution_code,
             reference_price,reference_price_time,proposed_quantity,reason)
            SELECT p.operation_id,p.strategy_id,e.event_id,e.paper_trade_id,e.entry_event_key,
             CASE WHEN (e.stock_code,e.direction) IS DISTINCT FROM
                            (p.stock_code,p.direction) THEN 'BLOCKED_MAPPING'
                  WHEN p.current_capital<=0 THEN 'BLOCKED_CAPITAL'
                  WHEN p.reference_price IS NULL OR p.reference_price<=0 THEN 'BLOCKED_REFERENCE'
                  ELSE 'BLOCKED_NO_SEND' END,
             p.execution_code,p.reference_price,p.reference_price_time,
             CASE WHEN p.reference_price>0
